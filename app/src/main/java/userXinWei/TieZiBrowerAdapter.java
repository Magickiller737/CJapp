...
public class TieZiBrowerAdapter  extends ArrayAdapter<Brower_pinlun> {
    //resourceID指定ListView的布局方式
    private int resourceID;
    private Context context;
    //重写BrowserAdapter的构造器
    public TieZiBrowerAdapter(Context context, int textViewResourceID , List<Brower_pinlun> objects){
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        //获取当前Browser实例
        Brower_pinlun brower = getItem(position);
        View view ;
        TieZiBrowerAdapter.ViewHolder viewHolder;
        //convertView为空则加载布局,不为空则重用
        //使用LayoutInfater为子项加载传入的布局
        if(convertView == null){
            view = LayoutInflater.from(getContext()).inflate(resourceID,null);
            viewHolder=new TieZiBrowerAdapter.ViewHolder();
            ...
            view = convertView;
            viewHolder = (TieZiBrowerAdapter.ViewHolder)view.getTag();//如果converView不为空说明这个布局已经加载过了，说明资源id已经绑定过了，那么直接使用getTag（）方法将这个绑定传入就行，不用再次绑定
        }
