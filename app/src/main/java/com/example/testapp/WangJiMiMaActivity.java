...
public class WangJiMiMaActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        yonghuming=findViewById(R.id.zhanghao);
        mima=findViewById(R.id.password_et);
    ...
    public void onClick(View view) {
        final String yonghuming1 = yonghuming.getText().toString();
        final String mima1 = mima.getText().toString();
        final String shoujihao1=shoujihao.getText().toString();

        if(view.getId()==R.id.button){//验证码
            //先验证手机号是否合法，如果合法则查询数据库
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            lianjie.ps.setString(1, yonghuming1);
                            lianjie.re = lianjie.ps.executeQuery();
                            if (lianjie.re.next()) {//如果有数据，说明用户名正确,则验证手机号是否绑定
                                ...
                                if(photo.equals(shoujihao1)){//手机号绑定正确，进入验证码阶段
                                    ...
                                    new Thread(new WangJiMiMaActivity.MyCountDownTimer()).start();//将按钮变为不可用60s
                                    Toast.makeText(WangJiMiMaActivity.this, "验证码已发送请耐心等待", LENGTH_SHORT).show();
                                    /**
                                     * 这里接下来的代码是跳转到一个专门的填写验证码的界面，填写正确则返回，并将全局变量里面是否填写正确验证码值置1
                                     */
                                    Intent intent = new Intent(WangJiMiMaActivity.this, YanZhengMaActivity.class);
                                ...
                                Toast.makeText(getApplicationContext(), "没有此用户", LENGTH_SHORT).show();
                                return;
                            }
                            new Thread(new WangJiMiMaActivity.Timer()).start();//为验证码设置60s的有效期
        ...
        }else if(view.getId()==R.id.queding){//确定
            if (QuanJU.getInstance().isYanzheng != 0) {//填写了验证码才能继续
                QuanJU.getInstance().isYanzheng=0;//将验证码归零
                if (mima1.matches("^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,16}$")) {//密码合乎规范，则继续修改
                    new Thread(new Runnable() {
                        @Override
                        public void run() {
                            Looper.prepare();
                            ...
                            try {
                                ...
                                lianjie.ps = lianjie.con.prepareStatement(sql);
                                lianjie.ps.setString(1,mima1);
                                lianjie.ps.setString(2,yonghuming1);
                                if(lianjie.ps.executeUpdate()>0){
                                    Toast.makeText(WangJiMiMaActivity.this, "修改成功，请重新登录", LENGTH_SHORT).show();
                                    Intent i = new Intent(WangJiMiMaActivity.this, MainActivity.class)
                                            .setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);//setFlags是跳转后清空之前的所有页面
                                    WangJiMiMaActivity.this.startActivity(i);
                                }else{
                                    Toast.makeText(WangJiMiMaActivity.this, "修改失败！！！", LENGTH_SHORT).show();
                                    finish();
                                }
                            } catch (ClassNotFoundException e) {
                                e.printStackTrace();
